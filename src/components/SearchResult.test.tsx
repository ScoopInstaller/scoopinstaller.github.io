...
describe('SearchResult', () => {
	...
	describe('command generation', () => {
		...
		it('should generate bucket command for community repository', () => {
			...
			const bucketCommand = screen.getByDisplayValue('scoop bucket add user_custom-bucket https://github.com/user/custom-bucket');
			expect(bucketCommand).toBeInTheDocument();
		});
	});
	...
	describe('result selection', () => {
		it('should call onResultSelected when name is clicked', async () => {
			...
			expect(mockOnResultSelected).toHaveBeenCalledWith(expect.objectContaining({
				name: 'nodejs',
			}));
		});

		it('should not make name clickable when onResultSelected is not provided', () => {
			const { container } = render(<SearchResult {...defaultProps} onResultSelected={undefined} />);
